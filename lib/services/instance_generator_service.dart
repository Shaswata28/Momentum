...
class InstanceGeneratorService {
  ...
  Future<void> generateForDate(DateTime date) async {
    ...
    // ── Sync pass: patch stale instances whose source RoutineTask was edited ──
    for (var task in routineTasks) {
      final staleInstances = existingInstances.where((i) =>
          i.routineTaskId == task.id && !i.isBufferBlock).toList();
      for (var instance in staleInstances) {
        // Skip already-completed/missed/skipped — don't overwrite user actions
        if (instance.status != TaskStatus.pending &&
            instance.status != TaskStatus.inProgress) continue;

        final needsUpdate = instance.title != task.title ||
            instance.scheduledTime != task.scheduledTime ||
            instance.flexWindowStart != task.flexWindowStart ||
            instance.flexWindowEnd != task.flexWindowEnd ||
            instance.durationMinutes != task.durationMinutes ||
            instance.enableDND != task.enableDND;

        if (needsUpdate) {
          final patched = DailyTaskInstance(
            id: instance.id,
            ...
            rescheduledToDate: instance.rescheduledToDate,
            isBufferBlock: instance.isBufferBlock,
            enableDND: task.enableDND,
            notificationId: instance.notificationId,
          );
          await instanceRepo.save(patched);
        }
      }
    }
